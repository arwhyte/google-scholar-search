...
class SoupScraper(Scraper):
    ...
    def get_citation(self, url: str, tag: Tag) -> Citation:
        ...
        return Citation(
            ...
            title_url=self.__select_value_by_key(tag.select_one(".gs_rt a"), "href"),
            pub_info=tag.select_one(".gs_a").text,
            author_links=self.__get_author_links(url, tag.find("div", {"class": "gs_a"})),
            snippet=tag.select_one(".gs_rs").text,
            cited_by=self.__select_value_by_key(
                tag.select_one("#gs_res_ccl_mid .gs_nph+ a"), "href"
            ),
            pdf_url=self.__select_value_by_key(
    ...
    def scrape(self):
        ...
        return [
            self.get_citation(self.base_url, tag).to_dict()
